...
def measure_object(frame, contours):
    ...
    object_count = 0  # Inicializa el contador de objetos

    for contour in contours:
        # Calcular área y perímetro para filtrar objetos pequeños
        area = cv2.contourArea(contour)
        if area < 500:  # Ajusta este valor según sea necesario
            continue
        ...
        object_count += 1  # Incrementar el contador de objetos

    # Mostrar el conteo total de objetos detectados
    cv2.putText(frame, f"Total Objetos: {object_count}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
while True:
    ...
    if calibrating:
        # Realizar zoom digital progresivo antes de la confirmación
        if zoom_factor != last_zoom_factor:
            last_zoom_factor = zoom_factor
            
        # Mostrar la ventana de confirmación después de aplicar el zoom
